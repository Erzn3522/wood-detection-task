...
static void draw_box_in_tile(cv::Mat& canvas, float board_x1, float board_y1, float board_x2,
                              float board_y2, const std::vector<int>& frame_widths,
                              int row_height, int cols, const cv::Scalar& color,
    ...
    for (int fi = 0; fi < static_cast<int>(frame_widths.size()); ++fi) {
        ...
        if (board_x2 > x_off && board_x1 < x_off + fw) {
            ...
            if (!label.empty()) {
                ...
                cv::putText(canvas, label, cv::Point(bx1, text_y), cv::FONT_HERSHEY_SIMPLEX,
                            ...
                            const std::filesystem::path& labels_dir, int cols)
{
    ...
    for (const auto& json_path : json_files) {
        ...
        for (int fi = 0; fi < n; ++fi) {
            ...
            cv::putText(canvas,
                        "#" + std::to_string(
                                  result.frames[fi].rfind('_') != std::string::npos
        ...
        for (size_t i = 0; i < result.knots.size(); ++i) {
            ...
            cv::Scalar  color = has_gt ? (tp_flags[i] ? cv::Scalar(0, 220, 0) : cv::Scalar(0, 220, 255))
                                       : cv::Scalar(0, 220, 0);
                              ...
                              int thickness, int dash = 7, int gap = 4)
{
    ...
    for (int i = 0; i < n_segs; ++i) {
        ...
        cv::line(img,
                 cv::Point(static_cast<int>(p1.x + t1 * dx), static_cast<int>(p1.y + t1 * dy)),
...
static void draw_dashed_rect_full(cv::Mat& img, cv::Point p1, cv::Point p2,
                           ...
                           const FrameStats& st, bool has_gt)
{
    ...
    const std::string row1 =
        fname + "   frame " + std::to_string(fi + 1) + "/" + std::to_string(n);
    ...
    if (!has_gt) {
        ...
        cv::rectangle(hdr, cv::Point(60, 24), cv::Point(69, 33), cv::Scalar(0, 220, 0),
                      cv::FILLED);
    ...
    const std::string cross_tp =
        st.tp_cross > 0 ? " (" + std::to_string(st.tp_cross) + "<>)" : "";
    const std::string cross_fp =
        st.fp_cross > 0 ? "(" + std::to_string(st.fp_cross) + "<>)" : "";

    const std::string row2 = "TP:" + std::to_string(st.tp) + cross_tp + "  FP:" +
                              std::to_string(st.fp) + cross_fp + "  FN:" + std::to_string(st.fn);
        ...
        for (int fi = 0; fi < n; ++fi) {
            ...
            for (size_t ki = 0; ki < result.knots.size(); ++ki) {
                ...
                if (has_gt) {
                    ...
                    if (tp_flags[ki]) {
                        ++stats.tp;
                        if (is_cross) ++stats.tp_cross;
                    } else {
                        ++stats.fp;
                        if (is_cross) ++stats.fp_cross;
                    }
                } else {
                ...
                cv::putText(canvas, lbl, cv::Point(lx1 + 2, text_y), cv::FONT_HERSHEY_SIMPLEX,
            ...
            if (has_gt) {
                for (size_t gi = 0; gi < gt.size(); ++gi) {
                    ...
                    if (gt_matched[gi]) {
                        ...
                        cv::putText(canvas, "FN",
                                    cv::Point(glx1 + 2, kHeaderH + gly1 + 12),
