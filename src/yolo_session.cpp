...
{
    const float scale =
        std::min(static_cast<float>(kInputSize) / bgr.cols, static_cast<float>(kInputSize) / bgr.rows);
                                     ...
                                     const std::string& device)
    {
        ...
        if (device == "gpu") {
            ...
            try {
                ...
                std::cerr << "[warn] CUDA EP unavailable: " << e.what() << " — falling back to CPU\n";
            }
        }
...
std::vector<Detection> YoloSession::predict(const cv::Mat& bgr_frame, float conf_threshold) const
{
    ...
    auto input_tensor = Ort::Value::CreateTensor<float>(
        memory_info, const_cast<float*>(chw.data()), chw.size(), input_shape.data(),
        input_shape.size());
